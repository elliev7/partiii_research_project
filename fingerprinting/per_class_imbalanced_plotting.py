...
def save_per_class_results_to_csv(coverage_results, accuracy_results, missed_results, limits, sample_size, name):
    ...
    df.to_csv(f"/home/ev357/tcbench/src/fingerprinting/results/automated_per_class/imb_{name}.csv")
...
def save_confusion_matrix(preds, trues, sample, limit, name_prefix=""):
    ...
    out_dir = f"/home/ev357/tcbench/src/fingerprinting/conf_matrices/automated_per_class/{name_prefix}"

    img_path = f"{out_dir}/imb_sample{sample}_limit{limit}.png"
    plt.savefig(img_path)
    plt.close()


with open("/home/ev357/rds/hpc-work/results/per_class/imb_embeddings_euclidean.pkl", "rb") as f:
    coverage_euc, accuracy_euc, missed_euc, preds_euc, trues_euc = pickle.load(f)

plot_results_per_class(coverage_euc, accuracy_euc, samples, percentiles, 'imb_automated_per_class_embeddings_splits_avg', percentile=True)
for s in samples:
    save_per_class_results_to_csv(coverage_euc, accuracy_euc, missed_euc, percentiles, s, f"embeddings_euclidean_sample{s}_avg")
for s in samples:
    for p in percentiles:
        save_confusion_matrix(preds_euc, trues_euc, s, p, name_prefix="Euclidean")


with open("/home/ev357/rds/hpc-work/results/per_class/imb_embeddings_cosine.pkl", "rb") as f:
    coverage_cos, accuracy_cos, missed_cos, preds_cos, trues_cos = pickle.load(f)

plot_results_per_class(coverage_cos, accuracy_cos, samples, percentiles, 'imb_automated_per_class_embeddings_cosine_splits_avg', percentile=True)
for s in samples:
    save_per_class_results_to_csv(coverage_cos, accuracy_cos, missed_cos, percentiles, s, f"embeddings_cosine_sample{s}_splits_avg")
for s in samples:
    for p in percentiles:
